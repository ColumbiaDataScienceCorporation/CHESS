...
def execute_model(
    predicted_sql, ground_truth, db_place, idx, iterate_num, meta_time_out, sql_dialect, sql_seq_no
):
    error = ''
    try:
    ...
    result = {"sql_idx": idx, "reward": reward, "err": error, "predicted": predicted_sql, "gold": ground_truth, "db": db, "seq": sql_seq_no}
    return result
...
def compute_ves_by_diff(exec_results, diff_json_path, sql_seq_no):
    num_queries = len(exec_results)
...
def dump_penalized_sqls(exec_result, args):
    wrong_sqls = []
    ...
    with open(str(Path(args.predicted_sql_path) / 'penalized_sqls.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    args_parser.add_argument("--iter_num", type=int, default=50)
    ...
    run_sqls_parallel(
        ...
        iterate_num=args.iter_num,
        meta_time_out=args.meta_time_out,
        sql_dialect=args.sql_dialect,
        sql_seq_no=sql_seq_no
    )
    exec_result = sort_results(exec_result)
    pd.DataFrame(exec_result).to_csv(Path(args.predicted_sql_path) / 'evaluation_ves_results.csv', index=False)
    dump_penalized_sqls(exec_result, args)
